...
bool                        g_ToggleFullScreen      = true;     //!< フルスクリーンモードにする場合は trueに設定
...
bool InitA3D()
{
    ...
    // スワップチェインの生成.
    {
        ...
        desc.EnableFullScreen   = g_ToggleFullScreen;
    ...
    // ディスクリプタセットレイアウトを生成します.
    {
        ...
        desc.Entries[0].ShaderStage    = a3d::SHADER_STAGE_VERTEX;
        desc.Entries[0].ShaderRegister = 0;
        desc.Entries[0].BindLocation   = 0;
        desc.Entries[0].Type           = a3d::DESCRIPTOR_TYPE_CBV;
...
void DrawA3D()
{
    ...
    if (changeMode)
    {
        ...
        g_pSwapChain->SetFullScreenMode(g_ToggleFullScreen);
    }
}
