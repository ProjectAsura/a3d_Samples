...
bool GuiMgr::Init(a3d::IDevice* pDevice, const TargetViewInfo& info, IApp* pApp)
{
    ...
    // 定数バッファを生成.
    {
        ...
        if ( !m_pDevice->CreateConstantBufferView(m_pConstantBuffer, &viewDesc, &m_pConstantView) )
        { return false; }
    ...
    // フォントテクスチャを生成.
    {
        ...
        cmdDesc.BufferSize  = 2048 + sizeof(a3d::DrawIndexedArguments) * MaxPrimitiveCount;
...
void GuiMgr::SwapBuffers()
{
    ...
    auto elapsedSec = float(elapsedMicroSec / (1000.0 * 1000.0));
